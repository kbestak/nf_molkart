...
def IlastikPrepOME(
    input, output, crop, crop_size, nonzero_fraction, nuclei_index, num_channels, channelIDs, crop_amount
):
...
def ParseInputOME():
    ...
    if args.nuclei_index is not None:
        nuc_idx = args.nuclei_index - 1
    else:
        nuc_idx = None
    if args.channelIDs is not None:
